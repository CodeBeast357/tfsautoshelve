...
namespace VsExt.AutoShelve.IO
{
    ...
    public class OptionsPageGeneral : DialogPage
    {
        ...
        [Category(GeneralCategory), DisplayName("Output Pane"), Description("Output window pane to write status messages.  If you set this to an empty string, nothing is written to the Output window.  Note: Regardless, the output pane is no longer explicitly activated.  So, no more focus stealing!")]
        public string OutputPane { get; set; }

        [Category(GeneralCategory), DisplayName("Μaximum Shelvesets"), Description("Maximum number of shelvesets to retain.  Older shelvesets will be deleted. 0=Disabled. Note: ShelvesetName must include a {2} (DateTime.Now component) unique enough to generate more than the maximum for this to have any impact.  If {0} (WorkspaceInfo.Name) is included, then the max is applied per workspace.")]
